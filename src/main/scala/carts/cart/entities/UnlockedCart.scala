...
object UnlockedCart {
  ...
  private case class UnlockedCartImpl(cartId: CartId, store: Store, isMovable: Boolean) extends UnlockedCart
  ...
  def apply(cartId: CartId, store: Store): UnlockedCart = UnlockedCartImpl(cartId, store, isMovable = true)
}
